...
Works with DocumentDB/MongoDB and PostgreSQL (Aurora and friends too). One core, three ways to use it: CLI, web UI, desktop app.
...
You need [uv](https://docs.astral.sh/uv/), plus mongosh and/or psql.
...
Any engine key that isn't `documentdb` gets treated as postgres-compatible, so aurora and such just work as extra engines (`warden pg --engine-key aurora ...`).
