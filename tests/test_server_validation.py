"""Unit tests for the validation / sanitisation helpers. Most live in
warden_core.validation (shared by the API handlers and the engine adapters); a
couple (sanitize_custom_config, get_config) are web-layer and stay in the server.
These sit in front of every query, so they're the security-critical layer."""

import pytest

from warden_web import server
from warden_core import validation as V


# ── mysql_account: name[@host] → quoted 'name'@'host' ──

def test_mysql_account_defaults_host():
    assert V.mysql_account("bob") == "'bob'@'%'"


def test_mysql_account_with_host():
    assert V.mysql_account("bob@10.0.0.1") == "'bob'@'10.0.0.1'"
    assert V.mysql_account("app@localhost") == "'app'@'localhost'"
...
def test_validate_mysql_privilege_ok(priv, expected):
    assert V.validate_mysql_privilege(priv) == expected


def test_validate_mysql_privilege_rejects_unknown():
    with pytest.raises(ValueError):
        V.validate_mysql_privilege("DROP DATABASE")


def test_validate_pg_privilege_ok():
    assert V.validate_pg_privilege("select") == "SELECT"


def test_validate_pg_privilege_rejects_unknown():
    with pytest.raises(ValueError):
        V.validate_pg_privilege("nonsense")


def test_validate_target_mysql_allows_host():
    assert V.validate_target("mysql", "bob@localhost") == "bob@localhost"


def test_validate_target_non_mysql_is_plain_ident():
    assert V.validate_target("postgresql", "bob") == "bob"
    with pytest.raises(ValueError):
        V.validate_target("postgresql", "bad name")   # space is not a valid ident


# ── validate_docdb_roles ──

def test_validate_docdb_roles_ok():
    V.validate_docdb_roles([{"role": "read", "db": "shop"}])   # no raise
...
def test_es_index_ok():
    assert V.es_index({"collection": "products"}) == "products"
...
def test_redis_key_ok():
    assert V.redis_key({"id": "cache:1"}) == "cache:1"
...
def test_is_mariadb_detects_and_caches(monkeypatch):
    ...
    monkeypatch.setattr(V, "my_query", fake_query)
...
def test_is_mariadb_false_for_mysql(monkeypatch):
    monkeypatch.setattr(V, "my_query", lambda *a: (0, "8.0.36\n", ""))
    V._MARIADB_CACHE.clear()
    assert V.is_mariadb({"host": "h2", "port": 3306}, "u", "p") is False
